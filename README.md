...
Here are the explanations to the fields in the jsonl file. Each json line corresponds to the set of ["clean"](#clean) questions (see below), corresponding to one article, written by one writer. 
...
- `questions`: A list of dictionaries explained below. Each line of json has a different number of questions, because we only include the ["clean"](#clean) questions in each line.
...
- `gold_label`: We can treat this label as the correct answer, given that only the "clean" questions are included in the dataset. 
- `writer_label`: The gold label the writer provided, verified by untimed validation. The number corresponds to the option number (1-indexed) in `options`. 
...
- `difficult`: A binary value. `0` means that more than 50% of the speed annotations answer the question correctly, so we claim the question is not difficult. Otherwise, the value is `1`.


### <a name="clean">What are the "clean" questions?</a>
...
### <a name="clean+difficult">What are the "clean+difficult" questions?</a>
